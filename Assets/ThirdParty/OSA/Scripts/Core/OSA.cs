...
namespace Com.TheFallenGames.OSA.Core
{
	...
	where TItemViewsHolder : BaseItemViewsHolder
	{
		...
		public double vpSize => _InternalState.vpSize;
		public double ctVirtualSize => _InternalState.ctVirtualSize;
		public double ctVirtualInsetFromVPS_Cached=> _InternalState.ctVirtualInsetFromVPS_Cached;
		...
		protected virtual void Awake() { if (!IsInitialized) Init(); }
		...
		public int GetFirstVisibleItemViewsHolderIndex()
		{
			...
			return _VisibleItems[0].ItemIndex;
		}

		public List<TItemViewsHolder> GetVisibleItems()
		{
		...
		public int GetMaxIndex()
		{
			...
			for (curVisibleIndex = 0; curVisibleIndex < _VisibleItemsCount; ++curVisibleIndex)
			{
				curItemViewsHolder = _VisibleItems[curVisibleIndex];
				curIndexInList = curItemViewsHolder.ItemIndex;
				max = curIndexInList > max ? curIndexInList : max;
			}
